...
5. **Anti-Patterns (CRITICAL)** - Check against ALL the **DON'T** guidelines in the ux skill. Look for AI slop tells (AI color palette, gradient text, glassmorphism, hero metrics, card grids, generic fonts) and general design anti-patterns (gray on color, nested cards, bounce easing, redundant copy).
...
- **Suggested command**: Which command to use (e.g., `/ux-normalize`, `/ux-optimize`, `/ux-harden`)
...
- "Use `/ux-normalize` to align components with design system (addresses 23 theming issues)"
- "Use `/ux-optimize` to improve performance (addresses 12 performance issues)"
- "Use `/ux-harden` to improve i18n and text handling (addresses 8 edge cases)"
