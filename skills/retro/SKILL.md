...
If found, the repo uses the [`jira` plugin](https://github.com/koolamusic/claudefiles/tree/main/plugins/jira) for sprint state. This skill **does not** read `.jira/sprints/*` files — that coupling lives inside `/jira:retro`, which knows how to interpret CONTEXT.md (locked decisions D-XX), VERIFICATION.md (goal-backward outcomes), and per-wave PLAN files.
