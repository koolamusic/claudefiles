...
Compile-time type assertions. No runtime — just the project's typecheck command. Catches regressions in generics, conditional types, and type constraints that runtime tests can't see.
...
Run with the project's typecheck command (`npx tsc --noEmit`, `bun typecheck`, etc.). If it compiles, it passes.
